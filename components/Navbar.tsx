  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-[var(--bg)]/85 backdrop-blur-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {/* Mobile nav row — pages visible at the top on small screens */}
        <nav className="-mx-4 flex items-center gap-1 overflow-x-auto border-t border-[var(--border)] px-4 py-1.5 md:hidden">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              onClick={() => setMobileOpen(false)}
              className="shrink-0 rounded-md px-2.5 py-1.5 font-mono text-[11px] text-[var(--text-2)] transition hover:bg-white/5 hover:text-[var(--accent)]"
            >
              {l.label}
            </Link>
          ))}
