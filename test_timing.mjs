...
// Architects, assembled from short chunks to keep the source clean
...
for (let i = 0; i < 3; i++) {
  console.log(`--- run ${i + 1} ---`);
  await timed("direct arcanine ", () => direct.readContract({ address: QUOTER, abi: QUOTER_ABI, functionName: "quoteExactInputSingle", args: [USDC, TOKEN, 1000000n, 10000, 0n] }));
  await timed("via Vercel proxy", () => viaProxy.readContract({ address: QUOTER, abi: QUOTER_ABI, functionName: "quoteExactInputSingle", args: [USDC, TOKEN, 1000000n, 10000, 0n] }));
  await timed("railway direct  ", () => railway.readContract({ address: QUOTER, abi: QUOTER_ABI, functionName: "quoteExactInputSingle", args: [USDC, TOKEN, 1000000n, 10000, 0n] }));
}
